...
import './GovermentList.css';

function GovermentalList({ geonameId }) {
  ...
  useEffect(() => {
    const urlProvinces = `https://secure.geonames.org/childrenJSON?geonameId=${geonameId}&username=melkamu`;
    axios
      .get(urlProvinces)
      .then((response) => {
        ...
        <div>
          {error ? (
            <p className="error-message">
              No Govermental divisions were found for this country or state.
            </p>
          ) : (
            <div>
              <h2 className="title-provinces-container">
                <img
                  src={getFlagImageUrl(provinceData[0].countryCode)}
                  alt="country-flag"
                  className="flag2"
                />
                {countryName}
...
GovermentalList.propTypes = {
  geonameId: PropTypes.string.isRequired,
};

export default GovermentalList;
